...
export const TASK_PROMPT_DEPTH_VALUES = ['none', 'fast', 'standard', 'full'];
export const TASK_PROMPT_COMPLEXITY_VALUES = ['low', 'medium', 'high'];
...
export function updateTaskPromptDepth({ threadId, cwd, codexHome, mode, complexity = 'high' } = {}) {
  const location = resolveTaskContext({ threadId, cwd, codexHome });
  if (!location) throw Error('需要有效任务 ID');
  if (!TASK_PROMPT_DEPTH_VALUES.includes(mode)) throw Error('未知拆提示词模式');
  if (!TASK_PROMPT_COMPLEXITY_VALUES.includes(complexity)) throw Error('未知提示词复杂度');
  const current = readTaskContext(location);
  if (!current && existsSync(location.summaryPath)) throw Error('现有摘要无效，未修改');
  const data = current?.data || {
    threadId: location.threadId, updatedAt: new Date().toISOString(),
  ...
  const updated = { ...data, updatedAt: new Date().toISOString(), prompt_depth_mode: mode };
  updated.prompt_description_complexity = complexity;
  if (mode === 'none') delete updated.processing_depth;
  else updated.processing_depth = mode;
  writeJsonAtomic(location.summaryPath, updated);
  return updated;
}

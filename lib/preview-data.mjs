...
function parseSessionOverview(lines, { threadId = null, title = "", updatedAt = null } = {}) {
  ...
  const completedMessages = Array.from(turns.values())
    ...
    .filter(Boolean);
  const latestTurnId = Array.from(turns.keys()).at(-1) || null;
  const latestTurn = latestTurnId ? turns.get(latestTurnId) : null;
...
async function readTaskContext(cwd, threadId, codexHome) {
  try {
    ...
    return {
      ...
      prompt_depth_mode: value.prompt_depth_mode,
      processing_depth: value.processing_depth,
      prompt_description_complexity: value.prompt_description_complexity,
      prompt_compilation_receipt: value.prompt_compilation_receipt && typeof value.prompt_compilation_receipt === "object"
        ? value.prompt_compilation_receipt : null,
      prompt_compilation_receipts: Array.isArray(value.prompt_compilation_receipts)
        ? value.prompt_compilation_receipts.slice(-12) : [],
...
async function walkSessionFiles(root, result) {
  ...
  await Promise.all(entries.map(async (entry) => {
    ...
    const current = threadId ? result.get(threadId) : null;
    if (threadId && (!current || entry.name.localeCompare(path.basename(current)) > 0)) result.set(threadId, entryPath);
  }));
}
...
export class PreviewRepository {
  ...
  async readPreview(rawId, title = "") {
    ...
    // The global session index can lag behind an actively written rollout
    // file. Prefer the newest trustworthy source so cards do not display an
    // old midnight timestamp while the conversation is still changing.
    const metadataMs = timestampValue(metadata?.updatedAt);
    const fileMs = Number.isFinite(currentStat.mtimeMs) ? currentStat.mtimeMs : 0;
    const updatedAt = new Date(Math.max(metadataMs, fileMs)).toISOString();
  ...
  async readOverview(rawId, title = "") {
    try {
      ...
      const metadata = this.metadataById.get(threadId);
      const metadataMs = timestampValue(metadata?.updatedAt);
      const fileMs = Number.isFinite(fileStat.mtimeMs) ? fileStat.mtimeMs : 0;
      const updatedAt = new Date(Math.max(metadataMs, fileMs)).toISOString();

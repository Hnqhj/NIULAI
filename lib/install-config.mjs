...
export function assetBrowserRuntime({ installDir, stateDir } = {}) {
  ...
  const serverPath = path.join(sourceRoot, 'server.cjs');
  return { sourceRoot, stateRoot, serverPath: existsSync(serverPath) ? serverPath : path.join(sourceRoot, 'server.js'), tokenPath: env.ASSET_BROWSER_TOKEN_FILE, env };
}

export async function ensureAssetBrowserState(runtime = assetBrowserRuntime()) {
  await mkdir(runtime.stateRoot, {recursive: true});
  const config = {enabled: true, sourceRoots: [], projects: [], storage: {stateRoot: runtime.stateRoot}, automation: {inbox: {enabled: false, capturePolicy: 'ticketed-only'}, routing: {enabled: false}}, deduplication: {enabled: false}};

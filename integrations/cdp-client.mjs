export async function readTargets(port = 9231) {
  let lastError = null;
  // Chromium may bind the remote debugger to IPv6 (::1) even when the
  // launch arguments request 127.0.0.1. Try both loopback families so the
  // launcher and injector agree on the same live target.
  ...
  throw lastError || new Error("CDP target list unavailable");
}

export function selectMainCodexTarget(targets) {
  return targets.find((target) =>
    target.type === "page"
      // Store builds currently expose the renderer title as "ChatGPT";
      // older builds used "Codex". Both are the main app target.
      && (target.title === "Codex" || target.title === "ChatGPT")
